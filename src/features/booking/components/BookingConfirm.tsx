...
import PayPalPaymentForm from '@/components/common/forms/PayPalPaymentForm';
import PhoneCodeSelect from '@/components/common/forms/PhoneCodeSelect';
        ...
        <div>
          ...
          <div className="flex gap-2">
            <PhoneCodeSelect
              ...
              onChange={(value) => onFormChange({ phoneCode: value })}
            />
            <input
              type="tel"
...
interface PaymentMethodSectionProps {
  method: 'sadad' | 'paypal';
  setMethod: (method: 'sadad' | 'paypal') => void;
}

function PaymentMethodSection({ method, setMethod }: PaymentMethodSectionProps) {
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="space-y-3">
        ...
        <div 
          ...
          className={`flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all ${
            method === 'sadad' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'
          }`}
        >
          <CreditCard className={`w-6 h-6 mr-4 ${method === 'sadad' ? 'text-indigo-600' : 'text-gray-400'}`} />
          <div className="flex-1">
            <p className={`font-medium ${method === 'sadad' ? 'text-gray-900' : 'text-gray-700'}`}>Credit / Debit Card</p>
            <p className="text-sm text-gray-500">Visa, Mastercard, QPAY via Sadad Qatar</p>
          </div>
          {method === 'sadad' && <Check className="w-6 h-6 text-indigo-600" />}
        </div>
        ...
        <div 
          ...
          className={`flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all ${
            method === 'paypal' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'
          }`}
        >
          <div className={`w-6 h-6 mr-4 flex items-center justify-center font-bold text-lg ${method === 'paypal' ? 'text-[#003087]' : 'text-gray-400'}`}>P</div>
          <div className="flex-1">
            <p className={`font-medium ${method === 'paypal' ? 'text-gray-900' : 'text-gray-700'}`}>PayPal</p>
            <p className="text-sm text-gray-500">Pay safely with your PayPal account</p>
          </div>
          {method === 'paypal' && <Check className="w-6 h-6 text-indigo-600" />}
        </div>
      </div>
...
function BookingConfirmContent() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'sadad' | 'paypal'>('sadad');
  ...
  const isValidBookingDates = () => {
    if (!bookingData?.booking?.checkIn || !bookingData?.booking?.checkOut) return false;
    
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const [inY, inM, inD] = bookingData.booking.checkIn.split('-').map(Number);
    const checkInDate = new Date(inY, inM - 1, inD);

    const [outY, outM, outD] = bookingData.booking.checkOut.split('-').map(Number);
  ...
  const datesValid = isValidBookingDates();
        ...
        <div className="max-w-7xl mx-auto px-4 pt-4">
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start">
            ...
            <p className="text-amber-700">
              Please check your dates. Check-in cannot be in the past, and check-out must be after check-in.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <PaymentMethodSection method={paymentMethod} setMethod={setPaymentMethod} />

            {/* Pay Button */}
            <button
              onClick={() => handlePayment(paymentMethod)}
              disabled={!isFormValid() || processing || !datesValid}
            ...
            >
              {processing ? (
                ...
                <>
                  {paymentMethod === 'sadad' ? 'Pay with Card' : 'Pay with PayPal'}
